...
#    keyshift = 0
    ...
    while (ourkey==0) and (keyshift < len(message)):
        for i in range (0, keyshift):
            strip[i] = message[i::keyshift]
        ...
        if len(keys) != 0:
            while count < len(message):
                shifted=shiftBy(message[count], 26 - keys[count%len(keys)])
